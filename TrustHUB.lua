...
gPreset:AddButton({ Text = "⚡ Low Resource (Multi-Account)", Func = function()
    -- Trim everything that costs CPU/RAM per instance so 3-4 clients survive
    -- on one PC: kill 3D rendering, delete the map, drop all visuals/panels.
    setTog("T_Disable3D", true)
    setTog("T_DeleteMap", true)
    setTog("T_RemoveFog", true)
    setTog("T_TitanESP", false)
    setTog("T_BossESP", false)
    setTog("T_KillNotif", false)
    setTog("T_StatsPanel", false)
    Library:Notify("Low Resource mode ON — 3D off, map deleted, visuals off. Hide the menu too (RightCtrl).", 7)
    task.delay(2, function() pcall(function() Library:Toggle(false) end) end)
end})
